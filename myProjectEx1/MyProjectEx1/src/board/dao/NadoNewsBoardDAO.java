...
public class NadoNewsBoardDAO {
	...
	public List<NadoNewsBoardDTO> list() {
		...
		setSql("SELECT no, title, writer, writedate, hit " + " FROM nadoboard ORDER BY no DESC");
	...
	public NadoNewsBoardDTO view(int no) {
		try {
			...
			setSql("SELECT no, title, content, writer, writedate, hit FROM nadoboard WHERE no = ?");
