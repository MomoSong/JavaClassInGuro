			...
			try {
				...
				case "2":
					...
					if (memManageUtil.nullCheck(joinId) == false)
						break;
					String joinPw = memManageUtil.rePatternCheck("^[\"\"#$*-_=+\\'\"]{6,16}*$",
							"\n비밀번호는 #$*-_=+'\"를 사용할 수 없습니다.\n비밀번호에 공백을 입력 할 수 없습니다.\n비밀번호는 최소 6자리 최대 16자리입니다.\n비밀번호를 입력하세요.",
							"비밀번호는 #$*-_=+'\"를 사용할 수 없습니다.\n비밀번호는 공백이 들어가서는 안 됩니다.\n다시 입력하세요.");
					if (memManageUtil.nullCheck(joinPw) == false)
						break;
					String joinName = memManageUtil.patternCheck("^{1,10}*$", "이름을 입력하세요.",
							"이름은 최소 1자 최대 10자 입력이 가능합니다.");
					if (memManageUtil.nullCheck(joinName) == false)
						break;
					String joinNick = memManageUtil.patternCheck("^{1,10}*$", "닉네임을 입력하세요.",
