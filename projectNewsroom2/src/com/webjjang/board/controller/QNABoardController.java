...
import com.webjjang.comment.service.QNACommentViewService;
		...
		try {	
			while (true) {
				...
				case "2":
					...
					// 댓글 데이터를 가져온다.
					QNACommentViewService qnaCommentViewService
					= new QNACommentViewService();
					QNACommentDTO qnaCommentDTO = qnaCommentViewService.process(no);
					// 가져온 댓글 데이터 출력
					QNACommentPrint qnaCommentPrint
					= new QNACommentPrint();
					qnaCommentPrint.view(qnaCommentDTO);
					break;
				case "3":
